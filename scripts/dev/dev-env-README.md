...
```bash
scripts/dev/dev.sh up        # 启动一切（中间件 + 后端三件套 + 前端）
scripts/dev/dev.sh status    # 查状态
scripts/dev/dev.sh logs gateway   # 看某个服务日志
scripts/dev/dev.sh down      # 全部停掉
```
...
之后 `scripts/dev/dev.sh up` 会找到本地镜像，不再触发 Docker Hub 拉取。
...
```bash
scripts/dev/dev.sh up                         # 默认：build + 起中间件 + 后端 + 前端
scripts/dev/dev.sh up --no-build              # 跳过 mvn package（jar 已存在）
scripts/dev/dev.sh up --no-frontend           # 不起前端 vite
scripts/dev/dev.sh down                       # 反向停掉一切
scripts/dev/dev.sh restart account            # 改完代码后重启单个 Java 服务
scripts/dev/dev.sh restart mw                 # 重启中间件 + 重新 seed Nacos
```
...
```bash
scripts/dev/dev.sh seed-nacos                    # 重推到本机 Nacos
scripts/dev/dev.sh restart account               # 让服务重新拉配置
```
...
A: 之前没 `down` 干净。`scripts/dev/dev.sh down` 后重试，仍占用就 `lsof -i :30260` 找进程。

**Q: 前端登录 401 / 403**
A: 默认 JPA 自动建表，但 `sys_user` 表是空的——通过前端注册一个，或在 Nacos 把 `register.enable: true`（默认已开）。

**Q: 想完全清空（包括 MySQL 数据）**
A: `scripts/dev/dev.sh nuke`（会删 docker volume）。
...
```bash
scripts/dev/dev.sh status                # 看哪个组件挂了
scripts/dev/dev.sh logs <service>        # 出问题就看日志（最后 50 行）
scripts/dev/dev.sh restart <service>     # 重启
```

Java 代码改动后流程：

```bash
scripts/dev/dev.sh build                 # 重新打 jar
scripts/dev/dev.sh restart <service>     # 重启对应服务
```
