...
template = template.gsub('${OPS_PROXY_SHARED_SECRET}', 'test-secret')
  ...
  .gsub('nacos.misu-server.svc.cluster.local:8848', '127.0.0.1:18848')
  .gsub('headlamp.kuboard.svc.cluster.local:80', '127.0.0.1:18080')
File.write(destination, template)
RB

docker run --rm -d --name "${CONTAINER}" --add-host host.docker.internal:host-gateway -p 18080:8080 \
  -v "${TMP_DIR}/nginx.conf:/etc/nginx/nginx.conf:ro" nginx:1.27-alpine nginx -g 'daemon off;' >/dev/null
for _ in {1..30}; do
  if curl -sS -o /dev/null -H 'Host: api.misu.chat' http://127.0.0.1:18080/_ops/healthz; then
    break
  fi
  sleep 0.2
done

code=$(curl -sS -o "${TMP_DIR}/unauthorized" -w '%{http_code}' -H 'Host: api.misu.chat' http://127.0.0.1:18080/nacos/)
[[ "${code}" == 401 ]]
code=$(curl -sS -o "${TMP_DIR}/nacos" -w '%{http_code}' -H 'Host: api.misu.chat' -H 'Cookie: MISU_OPS_SESSION=nacos-session' 'http://127.0.0.1:18080/nacos/v1/console/server/state?format=json')
[[ "${code}" == 200 ]]
rg -q 'NACOS_UPSTREAM path=/nacos/v1/console/server/state\?format=json' "${TMP_DIR}/nacos"
code=$(curl -sS -o "${TMP_DIR}/headlamp" -w '%{http_code}' -H 'Host: api.misu.chat' -H 'Cookie: MISU_OPS_SESSION=headlamp-session' 'http://127.0.0.1:18080/ops/headlamp/c/main/pods')
[[ "${code}" == 200 ]]
rg -q 'HEADLAMP_UPSTREAM path=/ops/headlamp/c/main/pods' "${TMP_DIR}/headlamp"
code=$(curl -sS -o /dev/null -w '%{http_code}' -H 'Host: api.misu.chat' http://127.0.0.1:18080/nacos)
[[ "${code}" == 308 ]]
code=$(curl -sS -o /dev/null -w '%{http_code}' -H 'Host: api.misu.chat' http://127.0.0.1:18080/unknown)
[[ "${code}" == 404 ]]
code=$(curl -sS -o /dev/null -w '%{http_code}' -H 'Host: attacker.example' http://127.0.0.1:18080/nacos/)
[[ "${code}" == 421 ]]
