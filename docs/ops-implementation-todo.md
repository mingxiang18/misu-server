...
- [x] 增加 `misu-ops-nacos-auth` Secret 模板引用及 Nacos 2.5.0 免二次登录说明；Headlamp 固定官方 v0.45.0 镜像摘要，使用 `-in-cluster` + 官方 `-proxy-auth=true` + `-unsafe-use-service-account-token`，由受信 sidecar 注入已校验用户身份并使用其自身 ServiceAccount/RBAC 访问集群 API；NodePort 已收回。
- [x] 收回 `kuboard/headlamp` 的 NodePort `30087`：JSON patch 将 Service 改为 ClusterIP 并显式移除 nodePort，保留名称、selector、port 和 targetPort；浏览器唯一入口为主站 ADMIN 代理路径。
