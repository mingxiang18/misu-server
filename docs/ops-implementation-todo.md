...
- [x] 增加 `misu-ops-nacos-auth` Secret 模板引用及 Nacos 2.5.0 免二次登录说明；Headlamp 固定官方 v0.45.0 镜像摘要，使用 `-in-cluster` + 官方 `-proxy-auth=true`，由受信 sidecar 注入已校验用户身份并保留其自身 ServiceAccount/RBAC。
