...
public class NatxClientHandler extends NatxCommonHandler {
    ...
    private final ConcurrentHashMap<String, NatxCommonHandler> channelHandlerMap = new ConcurrentHashMap<>();
    private final ChannelGroup channelGroup = new DefaultChannelGroup(GlobalEventExecutor.INSTANCE);

    public NatxClientHandler(int port, String secret, String proxyAddress, int proxyPort) {
        this.port = port;
        this.secret = secret;
...
//            log.info("Register to Natx server");
        } else {
            log.error("Register fail: " + natxMessage.getMetaData().get("reason"));
            ctx.close();
        }
    }
        ...
        try {
            ...
            localConnection.connect(proxyAddress, proxyPort, new ChannelInitializer<SocketChannel>() {
                ...
                public void initChannel(SocketChannel ch) throws Exception {
                    ...
                    ch.pipeline().addLast(
                            // todo 测试用，添加带宽限制处理器
//                            new ChannelTrafficShapingHandler(1024 * 1024, 1024 * 1024), // 限制为1MB/s
